"""BebopnetOrchestrator: per-theme full-then-slice (Bebopnet specifics).

Поведение идентично MINGUS, отличия — `cfg.bebopnet` блок и derived YAML.
"""
...
class BebopnetOrchestrator(BaseModelOrchestrator):
    name = "bebopnet"
        ...
        for theme_name, ts in manifest.themes.items():
            ...
            for idx in range(manifest.samples_per_theme):
                ...
                if sample is not None and sample.ok:
                    continue
                sample_dir = theme_dir / "bebopnet" / f"sample_{idx}"
        ...
        for theme_name, ts in manifest.themes.items():
            ...
            for sample_dir in sorted(model_dir.glob("sample_*")):
                ...
                try:
                    ...
                    print(f"[bebopnet] corrupted raw {raw}, trashing: {e}", file=sys.stderr)
                    if raw.exists():
                ...
                try:
                    ...
                    print(f"[bebopnet] post-process failed for {raw}: {e}", file=sys.stderr)
            ...
            for idx in range(cfg.samples_per_theme):
                ...
                if (theme_name, idx) in corrupted:
                    manifest.mark_sample(
                        theme_name, "bebopnet", idx, ok=False,
                        error="raw_full corrupted, trashed; will regenerate on next run",
                    )
                    continue
                if (sample_dir / "raw_full.mid").exists():
                    manifest.mark_sample(theme_name, "bebopnet", idx, ok=True)
                else:
                    ...
                    manifest.mark_sample(theme_name, "bebopnet", idx, ok=False, error=err)
